...
#set the default starting point for AUXSAMPE_FINE
backplane.setResistorRegister('AUXSAMPLE_FINE', 0)

# MAIN loop to capture data
while i < n:
	#set AUXSAMPLE_COARSE to i
	backplane.setResistorRegister('AUXSAMPLE_COARSE',i)
	#delay 0 seconds (default) or by number passed to the function
	time.sleep(delay)
	#Save the captured data to here using RAH function
	qemcamera.log_image_stream('/scratch/qem/coarse/adc_cal_AUXSAMPLE_COARSE_%04d' %i, frames)
	#increment i
	i=i+1
	print("%d/1024" %i)
